<?php
    ...
    if(isset($_GET['title'])) {
      ...
      {
        if(intval($review["user_id"]) == $_SESSION['userid'])
        {
          echo '<script>window.location.href="petitionnotallowed.php?title=' . $title . '";</script>';
        }
      }
      }
...
logout
    ...
    <div class="pet_img"><img src="petitionsupload/<?php echo $row["image_url"]; ?>" width="900px" height="500px"></div>
    <div class="pet_sec">
...
fingerprint
	...
	<div class="buttons">
		<input type="radio" name="vote" value="support" id="support-btn" class="support-btn" onclick="updateProgressBar(1)">
		<label for="support-btn" class="support-btn">Support</label>
		<input type="radio" name="vote" value="oppose" id="oppose-btn" class="oppose-btn" onclick="updateProgressBar(-1)">
		<label for="oppose-btn" class="oppose-btn">Oppose</label>
	</div>

	<!-- Support and oppose counters -->
    <form action="review.php?title=<?php echo $title;  ?>">
	<div class="counters">
		<span id="support-counter"><?php echo $row['supporters']; ?></span>
		<span id="oppose-counter"><?php echo $row['opposers']; ?></span>
	</div>
    ...
    <textarea placeholder="Write your review here" cols="40px" rows="5px" name="review" id="review" ></textarea>
...
function updateProgressBar(value) {
  ...
  data: {
    ...
    review: $('#review').val() // Get the review data from the textarea element
  },
